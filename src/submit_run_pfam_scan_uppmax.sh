...
usage="
usage: submit_run_pfam_scan_uppmax.sh fasta-seq-file [-outpath outpath] [-nseq INT] [-test]

    -nseq INT   Number of sequences per job, (default: $numseqperjob)
...
while [ "$1" != "" ]; do
    if [ "$isNonOptionArg" == "true" ]; then 
        ...
        case $1 in
            ...
            -nseq|--nseq) numseqperjob=$2;shift;;
        -test|--test) isTest=1;;
...
$DATADIR3/wk/MPTopo/src/run_pfam_scan.sh $splittedfastafile -outpath $splittedpath -r \"-pfamb\" > $logfile 2>&1" > $jobscriptfile
                if [ $isTest -eq 0 ];then
