...
#        print "Error"
    ...
    os.system("python my_uniqueseq.py " + work_dir +"QUERY.hits.db.temp")
...
#   print " - start pfamscan"
   if os.path.exists(work_dir + name  + ".txt") is False:
    ...
    timeout=open(work_dir+"Pscantimelog.txt","w")
    timeout.write("job_start: %.3f"%time.time()) 
    sCmd = "perl " + pfamScan + Clan_overlap + Eval_tr +" -fasta "+input_file + " -dir " + pfam_Dir + " -outfile " + work_dir + name  + ".txt"
    os.system(sCmd)
