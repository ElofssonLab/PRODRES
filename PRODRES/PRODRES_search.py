...
def PROTEIN_DOMAIN_REDUCED_SEARCH(env,inp):
    ...
    for entry in inp:
        ...
        # recursive report
        print("\t>Beginning PRODRES for sequence number "+str(counter)+"/"+str(input_length)+": "+name[:8])
        counter += 1
        ...
        if env.jackhmmer:
            ...
            if os.path.getsize(dbfile) == 0 and env.paramK:
                print("WARNING! Reduced DB is void, performing search in full DB")
                logging.warning("\t\t\t>Reduced DB found void, searching in full DB")
                dbfile = env.uniprot
...
def createHitDB(pfamList, work_dir,pfamseqdb):
    logging.info("\t\t\t> constructing PRODRES database")
